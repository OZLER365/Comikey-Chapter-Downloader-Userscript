...
(function() {
    ...
    function setActionState(state) {
        ...
        if (state === 'IDLE') {
            ...
            uiActionBtn.textContent = '⬇ Download Now';
            uiActionBtn.style.backgroundColor = '#2ed573';
            ...
            while (isAutoScrolling && currentMode === 'manga') {
                ...
                for (const container of visibleContainers) {
                    ...
                    while (waitAttempts < 150) {
                        ...
                        if (canvas) {
                            // If we already waited for this specific canvas on a previous loop, skip
                            ...
                            // Check if the lazy loader finally populated the height
                            if (hAttr && hAttr !== "0" && parseInt(hAttr, 10) > 0) {
                                canvas.dataset.scrollProcessed = "true";
                                newlyLoaded = true; // Flag that a new canvas just exposed its dimensions
                                break; 
                            }
                        }
                ...
                // If a canvas JUST updated its height to > 0, the image is fetching right now.
                // We MUST pause to give the browser time to download and draw before we scroll past it!
                if (newlyLoaded) {
                    await sleep(1500); 
                }
            ...
            while (isAutoScrolling && currentMode === 'webtoon') {
                ...
                for (const container of visibleContainers) {
                    ...
                    while (waitAttempts < 150) {
                        ...
                        // Break only when the explicit blob URL appears
                ...
                const totalElements = document.querySelectorAll('#br-spine > div').length || 0;
    ...
    async function triggerDownload() {
        ...
        uiDlBtn.textContent = 'Downloading...';
        ...
        if (currentMode === 'manga') {
            ...
            for (let i = 0; i < sortedBlobs.length; i++) {
                ...
                uiActionBtn.textContent = `Downloading ${i + 1} / ${sortedBlobs.length}`;
                uiDlBtn.textContent = `Downloading ${i + 1} / ${sortedBlobs.length}`;
            ...
            imagesToDownload.forEach((img) => {
                ...
                GM_download({
                    ...
                    onerror: (err) => { console.error(`Failed to dl ${fileName}`, err); URL.revokeObjectURL(blobUrl); }
                });
            });
            await sleep(2000); 
        }
